...
template <typename V>
inline int32_t
BinarySearch(const Array<KeyValuePair<std::string, V>> &array, uint32_t index, uint32_t count, const KeyValuePair<std::string, V> &item)
{
    int32_t left = index, right = index + count - 1;
    assert(left < array.size && right < array.size);
    while (left <= right) {
        uint32_t pivot = (left + right) >> 1;
        int cmp = strcmp(array[pivot].key.c_str(), item.key.c_str());
...
template <typename K, typename V>
inline
Dictionary<K, V>::Dictionary(Memory::Allocator &allocator)
: data(allocator),
  bulkAdding(false)
{ }
...
template <typename K, typename V>
inline
Dictionary<K, V>::Dictionary(Memory::Allocator &allocator, uint32_t initialCapacity)
