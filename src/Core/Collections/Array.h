...
template <typename T>
inline
Array<T>::Array(Memory::Allocator &_allocator)
: size(0),
  capacity(0),
  data(nullptr),
...
template <typename T>
inline
Array<T>::Array(Memory::Allocator &_allocator, uint32_t initialCapacity)
: size(0),
...
{
    return *const_cast<Memory::Allocator*>(allocator);
}
