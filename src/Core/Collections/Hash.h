...
template <typename T>
inline
Hash<T>::Hash(Memory::Allocator &allocator)
: map(allocator),
  entries(allocator),
  data(allocator)
{ }
...
template <typename T>
inline
Hash<T>::Hash(Memory::Allocator &allocator, uint32_t initialMapSize)
...
template <typename T>
inline Array<T*>
Hash<T>::GetAll(uint32_t key, Memory::Allocator &allocator)
{
...
template <typename T>
inline Array<T*>
Hash<T>::FindAll(uint32_t key, Memory::Allocator &allocator, std::function<bool(const T&)> f)
{
    Array<T*> array(allocator);
	HashEntry *entry = this->FindFirst(key);
    while (entry != nullptr) {
