...
public:
    Hash(Memory::Allocator &allocator);
    Hash(Memory::Allocator &allocator, uint32_t initialMapSize);
    ...
    Array<T*> GetAll(uint32_t key, Memory::Allocator &allocator);

    T* Find(uint32_t key, std::function<bool(const T&)> f);
    Array<T*> FindAll(uint32_t key, Memory::Allocator &allocator, std::function<bool(const T&)> f);
