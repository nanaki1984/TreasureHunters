...
String::String()
: heapBuffer(Memory::GetAllocator<Memory::MallocAllocator>()),
  length(0)
{
    localBuffer[0] = '\0';
}
...
String::String(Memory::Allocator &allocator, uint32_t initialSize)
: heapBuffer(allocator),
  length(0)
{
    localBuffer[0] = '\0';
...
String::String(Memory::Allocator &allocator, const char *string)
: heapBuffer(allocator),
  length(0)
{
    this->Set(string);
