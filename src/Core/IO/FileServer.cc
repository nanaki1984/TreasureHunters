...
DefineClassInfo(Core::IO::FileServer, Core::RefCounted);

FileServer::FileServer()
: aliases(GetAllocator<MallocAllocator>())
{
#if defined _WIN32
...
FileServer::ReadOnly(const char *pathToFile, BitStream &stream) const
{
    ...
    if (f == nullptr)
    {
        Core::Log::Instance()->Write(Core::Log::Warning, "Cound't read file \"%s\"", str.AsCString());
        return 1;
    }
...
FileServer::WriteOnly(const char *pathToFile, const BitStream &stream) const
{
    ...
    if (f == nullptr)
    {
        Core::Log::Instance()->Write(Core::Log::Warning, "Cound't write file \"%s\"", str.AsCString());
        return 1;
    }
