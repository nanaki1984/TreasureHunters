...
SmartPtr<RefCounted>
ClassInfo::Create(Memory::Allocator *allocator) const
{
    assert(ctor != nullptr);
...
: classesHash(GetAllocator<MallocAllocator>()),
  fccHash(GetAllocator<MallocAllocator>())// FAKE: GetAllocator returns an invalid reference
{ }
...
{
    classesHash = Collections::Hash<ClassInfo*>(GetAllocator<MallocAllocator>());
    fccHash = Collections::Hash<ClassInfo*>(GetAllocator<MallocAllocator>()); // REAL
